...
class DatabaseManager:
    ...
    def read_produit(self, produit_id=None):
        query = "SELECT produit.id, produit.nom, produit.description, produit.prix, produit.quantite, categorie.nom AS categorie FROM produit JOIN categorie ON produit.id_categorie = categorie.id"
        if produit_id:
            query += " WHERE id = %s"
